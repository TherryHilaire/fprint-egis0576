...
/*
 * Frames yielding fewer minutiae than this are partial or blurred presses.
 * Empirically (2026-08-22 score-harness, 35 frames): crisp presses yield
 * >=10 usable minutiae on this 70x57 sensor, partial/blurred ones <10.
 */
#define EGIS0576_MINUTIAE_MIN 10
...
/*
 * Runs NBIS minutiae detection on a canvas-sized image and returns the
 * number of detected minutiae, or -1 on error. Mirrors what
 * fp_image_detect_minutiae() does for FpImage objects.
 */
static gint
count_canvas_minutiae (const guchar *canvas)
{
  ...
  gint map_w, map_h, bw, bh, bd;
  LFSPARMS *lfsparms;
  guchar   *work;
  gint r;
  gint n = -1;

  work = g_memdup2 (canvas, EGIS0576_CANVAS_SIZE);
  lfsparms = g_memdup2 (&g_lfsparms_V2, sizeof (LFSPARMS));

  r = get_minutiae (&minutiae, &quality_map, &direction_map,
                    &low_contrast_map, &low_flow_map, &high_curve_map,
                    &map_w, &map_h, &binarized, &bw, &bh, &bd,
                    work, EGIS0576_CANVAS_WIDTH, EGIS0576_CANVAS_HEIGHT, 8,
                    0.0, lfsparms);

  if (r == 0 && minutiae)
    n = minutiae->num;

  g_clear_pointer (&minutiae, free_minutiae);
  g_clear_pointer (&binarized, g_free);
  ...
  g_clear_pointer (&lfsparms, g_free);
  g_clear_pointer (&work, g_free);
...
process_finger (FpDevice *dev, FpiUsbTransfer *transfer)
{
  ...
  guchar canvas[EGIS0576_CANVAS_SIZE];
  gint   nmin;

  upscale_and_pad_img (img, canvas);
  nmin = count_canvas_minutiae (canvas);
  fp_dbg ("Capture quality: %d minutiae detected", nmin);

  if (nmin < EGIS0576_MINUTIAE_MIN)
    {
      /* Partial or blurred press - ask the user to try again. */
      fpi_image_device_retry_scan (img_self, FP_DEVICE_RETRY_CENTER_FINGER);

      self->seq_type = SEQ_REPEAT;
      fpi_ssm_next_state_delayed (transfer->ssm, 100);
      return;
    }

  FpImage *fp_img = fp_image_new (EGIS0576_CANVAS_WIDTH, EGIS0576_CANVAS_HEIGHT);
  /* Sensor returns full image */
  memcpy (fp_img->data, canvas, EGIS0576_CANVAS_SIZE);
...
process_poll_transfer (FpDevice *dev, FpiUsbTransfer *transfer)
{
  ...
  /*
   * Live capture (2026-08-22, bcdDevice 15.72) showed the poll response is
   * SIGE REG VALUE STATUS with STATUS always 0x01, so buffer[6]&1 is
   * effectively always true. This is intentional: finger presence cannot be
   * read from registers on this device; it is decided from image content in
   * process_finger(). Keep the gate only to mirror the Windows driver flow.
   */
